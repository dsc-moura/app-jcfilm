...
public interface IAcessoRepository extends JpaRepository<Acesso, Long>{
	...
	List<Acesso> findAll();	
	@Query(nativeQuery = true, value = "SELECT a.* from jcf_tbl_cad_usuarios u INNER JOIN jcf_tbl_cad_acessos a ON u.id = a.id_usuario  WHERE u.email = :email AND a.permissao = true")
